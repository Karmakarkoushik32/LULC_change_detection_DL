...
All metrics are computed from an accumulated confusion matrix to avoid
floating-point issues that arise from averaging per-batch metrics.

Usage
-----
    meter = SegmentationMetrics(num_classes=5)
    for preds, targets in val_loader:
        meter.update(preds, targets)   # raw logits or argmax - both work
    report = meter.compute()
    meter.reset()

Metrics provided
----------------
  mIoU        : mean Intersection-over-Union across all classes
  Jaccard     : alias for IoU (same formula)
  F1 / Dice   : 2·TP / (2·TP + FP + FN) per class
  Precision   : TP / (TP + FP) per class
  Recall      : TP / (TP + FN) per class
  Pixel acc   : fraction of correctly classified pixels
  Per-class IoU table for logging / tensorboard
"""
...
def iou(confusion_mat, class_map):
    cm = confusion_mat.float()
    ...
    per_class = {
        class_map[i]: round(iou[i].item(), 4)
        for i in class_map
    }
...
def pixel_accuracy(confusion_mat):
    cm = confusion_mat.float()
    tp = torch.diag(cm)
    return (tp.sum() / cm.sum().clamp(min=1)).item()


def commission_error(confusion_mat, class_map):
    cm = confusion_mat.float()
...
def omission_error(confusion_mat, class_map):
    cm = confusion_mat.float()
